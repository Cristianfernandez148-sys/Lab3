...
def _metadata_key_for_image(image_key: str, metadata_prefix: str) -> str:
    # incoming/foo/bar.png -> metadata/foo/bar.png.json
...
def lambda_handler(event, context):
    ...
    for record in event.get("Records", []):
        ...
        # Loop prevention: ignore anything not in incoming/
        if not key.startswith("incoming/"):
            continue

        meta_key = _metadata_key_for_image(key, metadata_prefix)

        # Idempotency: if metadata already exists, do not reprocess  [oai_citation:16‡Lab_3.pdf](sediment://file_0000000082a471f584a6051fe1c4bbdf)
